...
export BRAWLTOME_NETWORK_NAME=${BRAWLTOME_NETWORK_NAME:-brawltome}
...
docker run --rm --entrypoint /bin/promtool \
	-v "$root/infra/observability/prometheus:/etc/prometheus:ro" \
...
docker run --rm --entrypoint /bin/amtool \
	-v "$root/infra/observability/alertmanager:/etc/alertmanager:ro" \
...
docker run --rm \
	-v "$root/infra/observability/loki/loki.yml:/etc/loki/loki.yml:ro" \
...
docker run --rm \
	-v "$root/infra/observability/tempo/tempo.yml:/etc/tempo/tempo.yml:ro" \
...
docker run --rm \
	-v "$root/infra/observability/otel-collector/config.yml:/etc/otelcol/config.yml:ro" \
	-v "$root/infra/observability/tests/fixtures/otel-ingest-token:/run/secrets/otel_ingest_token:ro" \
...
if grep -RniE 'discord(app)?\.com/api/webhooks|Bearer [A-Za-z0-9_-]{16,}' infra/observability \
