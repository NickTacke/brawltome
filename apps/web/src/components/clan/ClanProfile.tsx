...
import { ChevronLeft, Users, Trophy, Calendar, Crown, Shield, User, UserPlus, Search, TrendingUp, Clock } from 'lucide-react';
...
export function ClanProfile({ initialData: clan, id }: ClanProfileProps) {
    ...
    const getRankIcon = (rank: string) => {
        switch(rank.toLowerCase()) {
            ...
            case 'member': return <User className="w-4 h-4 text-emerald-400" />;
                            ...
                            <>
                                ...
                                <div className="flex items-center gap-2">
                                    <Badge variant="outline" className="text-xs font-mono text-muted-foreground gap-1.5 hover:bg-muted/50 transition-colors">
                                        ...
                                        <span className="hidden sm:inline">Updated </span>{timeAgo(clan.lastUpdated)}
                                    </Badge>
                                </div>
                            </>
                        )}
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {isLoading ? (
                                ...
                                <div className="text-2xl font-bold">{clan.clanXp ? parseInt(clan.clanXp).toLocaleString() : '0'}</div>
                            )}
                        </CardContent>
                    </Card>
                    <Card>
                        ...
                        <CardContent>
                            {isLoading ? (
                                ...
                                <div className="text-2xl font-bold">{clan.members?.length || 0}</div>
                            )}
                        </CardContent>
                    </Card>
                    <Card>
                            ...
                            {isLoading ? (
                                ...
                                <div className="text-2xl font-bold">
                                    {clan.members?.length > 0 
                                        ? Math.round(parseInt(clan.clanXp || '0') / clan.members.length).toLocaleString() 
                                        : '0'}
                                </div>
                            )}
                        </CardContent>
                            ...
                            {isLoading ? (
                                ...
                                Array.from({ length: 10 }).map((_, i) => (
                                    <TableRow key={i} className="border-border hover:bg-transparent">
                                        ...
                                        <TableCell className="p-4 flex justify-end"><Skeleton className="h-6 w-12" /></TableCell>
                                        <TableCell className="p-4"><Skeleton className="h-6 w-16 ml-auto" /></TableCell>
                                        <TableCell className="p-4 hidden sm:table-cell"><Skeleton className="h-6 w-24 ml-auto" /></TableCell>
                                    </TableRow>
                                ))
                            ) : (
                                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                paginatedMembers.map((member: any) => {
                                        ...
                                        <TableRow 
                                            ...
                                            onClick={() => router.push(`/player/${member.brawlhallaId}`)}
                                        >
                                            <TableCell>
