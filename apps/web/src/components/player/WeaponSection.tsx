...
import { Card, Button, Progress, Avatar, AvatarImage } from '@brawltome/ui'
import { ChevronDown, ChevronUp } from 'lucide-react'
import { getWeaponIcon, formatHours, formatCompact, WinLossBar } from './shared'
    ...
    <div ref={weaponsRef} className="space-y-4">
      ...
      <Card className="overflow-hidden border-border">
        {displayedWeapons.map((w) => {
          ...
          const avgElo = w.ranked.ratings.length > 0
            ? w.ranked.ratings.reduce((a, b) => a + b, 0) / w.ranked.ratings.length
            : 0
          const avgPeak = w.ranked.peakRatings.length > 0
                        ...
                        <div className="space-y-3 md:pr-4 md:border-r md:border-border/30">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="flex justify-between text-[10px] font-bold">
                              <span className="text-foreground">{formatNum(w.wins)}W <span className="font-normal text-muted-foreground">({winrate.toFixed(1)}%)</span></span>
                              <span className="text-foreground">{formatNum(w.games - w.wins)}L <span className="font-normal text-muted-foreground">({(100 - winrate).toFixed(1)}%)</span></span>
                            </div>
                          </div>
                          ...
                          {w.ranked.games > 0 ? (
                            <div className="space-y-3 mt-3">
                              ...
                              <div className="space-y-1.5">
                                ...
                                <div className="flex justify-between text-[10px] font-bold">
                                  <span className="text-foreground">{formatNum(w.ranked.wins)}W <span className="font-normal text-muted-foreground">({rankedWinrate.toFixed(1)}%)</span></span>
                                  <span className="text-foreground">{formatNum(w.ranked.games - w.ranked.wins)}L <span className="font-normal text-muted-foreground">({(100 - rankedWinrate).toFixed(1)}%)</span></span>
                                </div>
                              </div>
