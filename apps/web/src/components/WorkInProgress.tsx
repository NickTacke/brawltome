...
export function WorkInProgress({ slug }: { slug: WipFeatureSlug }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-6.5rem)] flex-col items-center justify-center text-center sm:min-h-[calc(100dvh-7rem)] md:min-h-[calc(100dvh-4.5rem)]">
      ...
      <p className="text-muted-foreground mt-8 text-xs font-semibold uppercase tracking-widest">Coming soon</p>

      <h1 className="text-foreground mt-2 text-5xl font-bold tracking-tight">{feature.title}</h1>

      <p className="text-muted-foreground mt-4 max-w-md text-xl">{feature.tagline}</p>

      <p className="text-muted-foreground mt-6 max-w-md text-base leading-relaxed">{feature.description}</p>
