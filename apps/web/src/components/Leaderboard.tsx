...
function SortableHeader({ label, sortKey, currentSort, currentOrder, onSort, className }: {
  label: string; sortKey: string; currentSort: string; currentOrder: SortOrder; onSort: (key: string) => void; className?: string
}) {
  const isActive = currentSort === sortKey
  return (
    <TableHead className={className}>
      <button onClick={() => onSort(sortKey)} className="flex items-center gap-2 w-full hover:text-primary transition-colors font-bold">
        {label}
        {isActive ? (currentOrder === 'asc' ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />) : <ArrowUpDown className="h-4 w-4 opacity-30" />}
      </button>
    </TableHead>
  )
}

function PaginationControls({ page, isLoading, onPageChange, compact = false }: {
  page: number; isLoading: boolean; onPageChange: (page: number) => void; compact?: boolean
}) {
  return (
    <div className="flex items-center gap-2">
      <Button variant="outline" size="sm" disabled={page === 1 || isLoading} onClick={() => onPageChange(Math.max(1, page - 1))}>
        {compact ? '\u2190' : '\u2190 Prev'}
      </Button>
      <div className="flex items-center gap-2">
        ...
        <Input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              ...
              else { onPageChange(1); e.currentTarget.value = '1' }
            }
          }}
        />
    ...
    <Card className="w-full max-w-5xl mx-auto mt-12 bg-card/50 backdrop-blur-xs border-border">
      <div className="p-6 border-b border-border flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <PaginationControls page={page} isLoading={isLoading} onPageChange={(p) => updateQueryParams({ page: p })} compact />
        </div>
      </div>
        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="border-border hover:bg-transparent">
              ...
              <SortableHeader label="Rating" sortKey="rating" currentSort={sortBy} currentOrder={sortOrder} onSort={handleHeaderSort} className="text-center" />
              <TableHead className="text-center font-bold">Win Rate</TableHead>
              <SortableHeader label="Wins" sortKey="wins" currentSort={sortBy} currentOrder={sortOrder} onSort={handleHeaderSort} className="text-center hidden sm:table-cell" />
              <SortableHeader label="Games" sortKey="games" currentSort={sortBy} currentOrder={sortOrder} onSort={handleHeaderSort} className="text-center hidden sm:table-cell" />
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              Array.from({ length: 10 }).map((_, i) => (
                <TableRow key={i} className="border-border hover:bg-transparent">
                  ...
                  <TableCell className="p-4"><Skeleton className="h-5 w-32 mb-2" /><Skeleton className="h-3 w-20" /></TableCell>
              ...
              bracket === '1v1' ? (
                ...
                entries.map((p: any, i: number) => {
                  ...
                  return (
                    <TableRow key={p.brawlhallaId} className="border-border cursor-pointer transition-colors group h-16">
                      <TableCell className={`p-0 text-center ${getRankStyle(globalRank)}`}>
                        <Link href={href} prefetch={false} className="block w-full h-full p-4">#{globalRank}</Link>
                      </TableCell>
                      ...
                      <TableCell className="p-0 text-center">
                        <Link href={href} prefetch={false} className="block w-full h-full p-4">
                          <div className={`font-bold ${winrate >= 60 ? 'text-success' : winrate >= 50 ? 'text-primary' : 'text-muted-foreground'}`}>{winrate.toFixed(1)}%</div>
                        </Link>
                      </TableCell>
                      <TableCell className="p-0 text-center hidden sm:table-cell text-muted-foreground font-mono">
                        <Link href={href} prefetch={false} className="block w-full h-full p-4">{wins}</Link>
                      </TableCell>
                      <TableCell className="p-0 text-center hidden sm:table-cell text-muted-foreground font-mono">
                        <Link href={href} prefetch={false} className="block w-full h-full p-4">{games}</Link>
                      </TableCell>
                    </TableRow>
                  )
                })
                ...
                entries.map((t: any) => {
                  ...
                  return (
                    <TableRow key={`${t.brawlhallaIdOne}-${t.brawlhallaIdTwo}`} className="border-border transition-colors group h-16">
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          <div className="font-bold text-foreground text-base max-w-[420px] md:max-w-[560px] whitespace-normal leading-tight">
                            <Link href={`/player/${t.brawlhallaIdOne}`} prefetch={false} className="hover:text-primary">{fixEncoding(t.playerOneName || 'Unknown')}</Link>
                            <span className="opacity-50"> + </span>
                            <Link href={`/player/${t.brawlhallaIdTwo}`} prefetch={false} className="hover:text-primary">{fixEncoding(t.playerTwoName || 'Unknown')}</Link>
                          </div>
                          <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground font-mono">
                            <span>{t.region}</span>
                            {t.tier && <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-5 font-normal bg-muted text-muted-foreground border-border">{t.tier}</Badge>}
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-center hidden sm:table-cell text-muted-foreground font-mono">{t.wins}</TableCell>
                      <TableCell className="text-center hidden sm:table-cell text-muted-foreground font-mono">{t.games}</TableCell>
                    </TableRow>
                  )
                })
              )
