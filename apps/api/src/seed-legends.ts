...
async function bootstrap() {
    ...
    try {
        ...
        logger.log('📥 Fetching all legends summary...');
        const allLegends = await bhApiClient.getAllLegends();
        
        logger.log(`Found ${allLegends.length} legends. Starting detailed fetch...`);

        // 2. Iterate and fetch details for each
        for (const summary of allLegends) {
            try {
                logger.log(`Fetching details for ${summary.bio_name} (ID: ${summary.legend_id})...`);
